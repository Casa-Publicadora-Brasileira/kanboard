<article class="ca-task-panel-content" aria-labelledby="ca-task-panel-title">
    ...
    <section class="ca-panel-section ca-panel-details">
        ...
        <dl class="ca-detail-grid">
            ...
            <div><dt><?= t('CompanyAgile: Assignee') ?></dt><dd><?= empty($task['owner_id']) ? t('CompanyAgile: Unassigned') : $this->text->e($task['assignee_name'] ?: $task['assignee_username']) ?></dd></div>
            ...
            <div class="ca-detail-wide"><dt><?= t('CompanyAgile: Tags') ?></dt><dd><?php if (empty($tags)): ?>—<?php else: ?><?php foreach ($tags as $tag): ?><span class="ca-tag <?= (int) $tag['project_id'] > 0 ? 'ca-tag-project' : 'ca-tag-global' ?>"><?= $this->text->e($tag['name']) ?></span><?php endforeach ?><?php endif ?></dd></div>
        </dl>
    </section>
